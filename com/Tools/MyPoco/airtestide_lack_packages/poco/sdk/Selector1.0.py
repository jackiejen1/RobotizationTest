...
class Selector(ISelector):
    ...
    def getRoot(self):
        ...
        lits = []
        lits = self.get_ui_tree(self.dumper.getRoot().__dict__,"",lits)
        with open(file_path, 'w') as f:
            f.write(str(lits))
        # cf = configparser.ConfigParser()
        # root_dir = r"D:\yoozoo_gitlab\autotest\Tools\poco_ui_list.ini"
        # cf.read(root_dir)
        # cf.set("PocoUIList", "finduilist", str(lits))
        # cf.write(open(root_dir, "w"))
        return self.dumper.getRoot()

    def get_ui_tree(self, dic, ui_path, ui_path_list):
        dics = dic
        keys = dic.keys()
        if 'node' in keys:
            dics = dic['node']
            keys = dics.keys()
        ui_patha = dics['name']
        ui_pathb = ui_path+ui_patha
        ui_path_list.append(ui_patha)
        ui_path_list.append(ui_pathb)

        if "text" in dics['payload'].keys():
            ui_pathc = dics['payload']['text']
            ui_path_list.append(ui_pathc)
        if 'children'in keys:
            childs_list = dics['children']
            ui_pathb = ui_pathb+"/"
            for i in range(len(childs_list)):
                childs_dic = childs_list[i]
                ui_path_list = self.get_ui_tree(childs_dic, ui_pathb, ui_path_list)
        return ui_path_list
