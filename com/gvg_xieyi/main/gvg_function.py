...
class LoginGame:
    def __init__(self, server_id):
        ...
        self.gate_dic = {}#城门编号和城门位置id
        self.go_gate_dic={}#城门位置id和可攻击位置id
        self.buff_dic = {}#buff点编号和[位置，buff_id，消失时间]
        self.tower_dic = {}#炮台编号和[城门位置id,军团id(可选)]
    ...
    def MSG_C2S_GVG_EnterMatch(self, ):
        ...
        for user_list in S2C_GVG_EnterMatch.users:  # 比赛中的玩家列表
            if "pos_id" in dir(user_list):
                pos_id = user_list.pos_id
            else:
                pos_id = 0
            if "move_arrive_time" in dir(user_list):
                move_arrive_time = user_list.move_arrive_time
            else:
                move_arrive_time = 0
            self.match_user_dic[user_list.user.id] = {"军团ID":user_list.user.guild_id,"位置ID":pos_id,"到达时间":move_arrive_time}
        gate_num = 1
        for gate_list in S2C_GVG_EnterMatch.gates:  # 城门
            self.gate_dic[gate_num] = [gate_list.grid_id, gate_list.hp]
            gate_num = gate_num + 1
        buff_num = 1
        for buff_list in S2C_GVG_EnterMatch.buffs:  # BUFF点
            ...
            self.buff_dic[buff_num] = [grid_id, buff_id, end_time]
            buff_num = buff_num + 1
        tower_num = 1
        for tower_list in S2C_GVG_EnterMatch.towers:  # 炮台
            ...
            if "guild_id" in dir(tower_list):
                ...
                self.tower_dic[tower_num_str] = [grid_id, guild_id]
            else:
                self.tower_dic[tower_num_str] = [grid_id]
    ...
    def waiting_for_you(self,recvcmd_into,data):
        ...
        if recvcmd_into == 14444:
            ...
            #收到推送后自动更新场上玩家列表
            self.match_user_dic[S2C_GVG_NotifyUserEnter.user.user.id] = S2C_GVG_NotifyUserEnter.user.user.guild_id
        ...
        if recvcmd_into == 14464:
            ...
            #出现buff了，就刷新buff
            self.buff_dic[S2C_GVG_NotifyNewBuff.grid_buff.grid_id] = [S2C_GVG_NotifyNewBuff.grid_buff.buff_id,S2C_GVG_NotifyNewBuff.grid_buff.end_time]
    ...
    def MSG_C2S_GVG_Move(self, target_pos_id, cost_time):
        """
        军团战-玩家移动
        :param uid:
        :param sid:
        ...
        C2S_GVG_Move.cost_time = cost_time
    ...
    def MSG_C2S_GVG_AddBuff(self, ):
        ...
        C2S_GVG_AddBuff_attr = {'name': "C2S_GVG_AddBuff", 'protocol': 'protobuf-ss',
                                'send_cmd': 14460, 'recv_cmd': [14461, 14462], 'uid': self.uid, 'sid': self.sid}
...
class MyTaskSet(TaskSequence):
    ...
    def go_run(self):
        ...
        lg.MSG_C2S_GVG_Join()  # 报名 ok
        lg.MSG_C2S_GVG_GetInfo()  # 获取军团战信息  ok

        lg.MSG_C2S_GVG_EnterMatch()  # 进入比赛 ok  公会列表
        # 移动
        # 玩家战斗，抢炮台
        # 领取buff
        # 打城门

        lg.MSG_C2S_GVG_GetMatchInfo()  # 获取军团战比赛信息， ok
        lg.MSG_C2S_GVG_GetAchieve()  # 获取赛季成就信息  todo 协议有更新
        if len(self.achieve_id_list)>0:
            for achieve_id in self.achieve_id_list:
                lg.MSG_C2S_GVG_GetAchieveAward(achieve_id)  # 领取赛季成就奖励  ok

        lg.MSG_C2S_GVG_GetKingHistory()  # 获取历代王者  ok
        lg.MSG_C2S_GVG_LeaveMatch()  # 离开比赛  ok
        target_pos_id = 0  # 地图坐标
        cost_time = 0  # 移动时间
        lg.MSG_C2S_GVG_Move(target_pos_id, cost_time)  # 玩家移动
        user_id = 0  # 玩家UID todo 需要一个大列表，里面有所有军团的所有玩家id
