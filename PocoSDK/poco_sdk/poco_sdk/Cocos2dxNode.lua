...
function Node:getChildren()
    ...
    for i = 1, count do
        local child = childs:objectAtIndex(i-1)
...
function Node:getAttr(attrName)
    ...
    elseif attrName == 'name' then
        ...
        local name = self.node.getName and self.node:getName() or ''
        if name == '' or  string.match(name, "unknown") then
            name = "未命名"--self.node:getDescription()
        end
        return name
    ...
    elseif attrName == 'pos' then
        -- 转换成归一化坐标系，原点左上角
        -- local pos = self.node:convertToWorldSpaceAR(ccp(0, 0))
    ...
    elseif attrName == 'zOrders' then
        local zOrders = {
            global = self.node:getZOrder(),
            ['local'] = self.node:getZOrder(),
        }
        return zOrders
