...
function PocoManager:init_server(port)
    ...
    release_print(string.format('[poco] server listens on tcp://*:%s', port))
    ...
    self.timer = cc.Director:getInstance():getScheduler():scheduleScriptFunc(function() self:server_loop() end, 0.025, false)
end
    ...
    if #r > 0 then
        ...
        for i, v in ipairs(r) do
            if v == self.server_sock then
                ...
                if client_sock~=nil then
                    release_print('[poco] new client accepted', client_sock:getpeername(), err)
...
function PocoManager:onRequest(req)
    ...
    if func == nil then
        ...
        g.log:info("onRequest: ",  ret.error.message)
        client:send(ret)
    else
        xpcall(function()
